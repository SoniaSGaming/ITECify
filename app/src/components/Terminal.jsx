...
const xtermThemes = {
  ...
  'hc-black': {
    ...
    cursor: '#ffffff', selectionBackground: '#ffffff30',
    black: '#000000', red: '#f44747', green: '#89d185',
    yellow: '#d7ba7d', blue: '#569cd6', magenta: '#c586c0',
    cyan: '#4ec9b0', white: '#ffffff',
  },
};

function TerminalComponent({ onOpenFile, theme = 'vs-dark', wsRef }) {
  ...
  useEffect(() => {
    ...
    // 👇 expose ws to parent so Run button can send commands
    if (wsRef) wsRef.current = ws;

    ws.onopen = () => {
      term.write('\r\n\x1b[32mConnected\x1b[0m\r\n');
      ws.send(new TextEncoder().encode('export PROMPT_COMMAND="pwd > /tmp/cwd"\n'));
    };
    ...
    ws.onmessage = e => {
      ...
      fetch('/api/file', {
        ...
        body: JSON.stringify({ path: '/tmp/cwd' }),
      }).then(r => r.json()).then(d => {
        if (d.content) cwd.current = d.content.trim();
      }).catch(() => {});
    };
    ...
    term.onData(async data => {
      if (data === '\r') {
        ...
        if (cmd.startsWith('open ')) {
          ...
          const fullPath = filePath.startsWith('/') ? filePath : `${cwd.current}/${filePath}`;
          try {
            ...
            onOpenFile(fullPath, fileData.content);
            term.write('\r\n\x1b[32mOpened: ' + fullPath + '\x1b[0m\r\n');
          } catch {
            term.write('\r\n\x1b[31mCould not open: ' + fullPath + '\x1b[0m\r\n');
          }
          return;
        }
