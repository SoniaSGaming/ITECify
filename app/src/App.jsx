...
function App() {
  ...
  const [bottomTab, setBottomTab] = useState("terminal"); // "terminal" | "output"
  ...
  function showSuggestionWidget(lineNumber, cod) {
    ...
    acceptBtn.onclick = () => {
      ...
      editor.executeEdits('suggestion', [{ range: new monaco.Range(insertLine, endCol, insertLine, endCol), text: textToInsert }]);
      editor.setPosition({ lineNumber: insertLine + cod.split('\n').length, column: 1 });
      editor.focus();
      removeSuggestionWidget();
    };
    ...
    label.style.cssText = 'color:#aaa;font-size:11px;flex:1;font-family:monospace;';
  ...
  function handleEditorChange(value) {
    ...
    debounceTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        const lineNumber = isNaN(rawLine) || rawLine < 1 || rawLine > totalLines
          ? editorRef.current.getPosition().lineNumber : rawLine;
  ...
  async function runCode(lang, code) {
    ...
    if (!config) return { type: "error", content: `Language "${lang}" not supported` };
    const res = await fetch("/piston/api/v2/execute", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        language: config.language, version: config.version,
        files: [{ name: lang === "cpp" ? "main.cpp" : "main", content: code }],
      }),
    });
    const data = await res.json();
    return { type: "text", content: data.run.stderr || data.run.stdout || "(no output)" };
  }

  async function handleRun() {
    const code = editorRef.current.getValue();

    if (showBottomPanel && bottomTab === "terminal" && termWs.current?.readyState === WebSocket.OPEN) {
      const ext = { javascript: 'js', python: 'py', cpp: 'cpp', php: 'php', html: 'html' }[language] || 'txt';
      const tmpFile = `/tmp/run_${Date.now()}.${ext}`;
      const runners = {
        python: `python3 ${tmpFile}`, javascript: `node ${tmpFile}`,
        cpp: `g++ ${tmpFile} -o /tmp/run_out && /tmp/run_out`,
        php: `php ${tmpFile}`, html: `xdg-open ${tmpFile}`,
      };
      const runner = runners[language];
      if (!runner) return;
      await fetch('/api/file/save', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: tmpFile, content: code }),
      });
    ...
    setBottomTab("output"); // auto-switch to output tab
    setIsRunning(false);
  }
  ...
  return (
    <div className="screen">
      ...
      <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button
          onClick={() => {
            if (!showBottomPanel) { setShowBottomPanel(true); setBottomTab("terminal"); }
            else if (bottomTab !== "terminal") { setBottomTab("terminal"); }
          ...
          {showBottomPanel && (
            <div style={{
              ...
              <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
                {/* Terminal — always mounted so the pty session persists, just hidden when not active */}
                <div style={{
                  ...
                  <TerminalComponent
                    ...
                    onOpenFile={(path, content) => {
                      ...
                      const extMap = { py: 'python', js: 'javascript', cpp: 'cpp', php: 'php', html: 'html' };
                ...
                {bottomTab === "output" && (
                  <div style={{ position: "absolute", inset: 0, overflow: "auto" }}>
                    {!output && (
                      <div style={{ color: "#555", fontFamily: "monospace", fontSize: "12px", padding: "12px" }}>
                    ...
                    {output?.type === "error" && (
                      <pre style={{
                        margin: 0, padding: "10px 14px",
                        color: "#f44747", fontFamily: "monospace",
                        fontSize: "12.5px", lineHeight: "1.6",
                        whiteSpace: "pre-wrap", wordBreak: "break-word",
                      }}>
                        {output.content}
